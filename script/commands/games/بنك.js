...
async function createCard(data) {
  ...
  // ═════════════════ 1. خلفية سائلة داكنة مع توهج مركزي ═════════════════
  ...
  // ═════════════════ 2. إطار زجاجي كبير مع حواف دائرية وتوهج خارجي ═════════════════
  ...
  // ═════════════════ 3. الصورة الرمزية مع تأثير الهولوجرام ═════════════════
  const avatarX = 200;
  const avatarY = 275;
  ...
  try {
    ...
    // رسم أيقونة افتراضية في حال فشل تحميل الصورة
    ctx.save();
    ctx.shadowColor = theme.neon;
    ctx.shadowBlur = 40;
    ctx.fillStyle = theme.dark;
  ...
  // ═════════════════ 4. الاسم والرتبة مع تأثيرات النيون ═════════════════
  ctx.textAlign = "left";
  ctx.textBaseline = "top";

  // اسم المستخدم كبير مع ظل طويل
  ctx.save();
  ctx.shadowColor = theme.neon;
  ctx.shadowBlur = 30;
  ctx.font = "bold 58px 'Arial Black', 'Helvetica Bold', sans-serif";
  ...
  stats.forEach((stat, i) => {
    ...
    ctx.beginPath();
    ctx.roundRect(statsX - 10, y - 5, 500, 45, 10);
    ctx.fill();

    // الأيقونة
    ctx.font = "30px 'Segoe UI Emoji', 'Arial'";
    ctx.fillStyle = theme.accent;
    ctx.fillText(stat.icon, statsX, y);

    // التسمية
    ctx.font = "bold 26px 'Arial'";
    ctx.fillStyle = "#aaa";
    ctx.fillText(stat.label, statsX + 60, y + 2);
    ...
    ctx.shadowColor = theme.neon;
    ctx.shadowBlur = 20;
    ctx.fillText(stat.value, statsX + 300, y);
    ctx.shadowBlur = 0;
  });

  // ═════════════════ 6. شريط الخبرة المتوهج (بصمة مستقبلية) ═════════════════
  const barX = 420;
  const barY = 460;
  const barW = 800;
  const barH = 24;
  ...
  // نسبة التقدم داخل الشريط (للمحترفين)
  ...
  // ═════════════════ 9. نسيج ضوضاء خفيف جداً ═════════════════
  applyNoise(ctx, W, H);

  return canvas.toBuffer("image/png");
}

// Helper لرسم مستطيل مدور
